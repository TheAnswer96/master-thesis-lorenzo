...
def to_adj_matrix(voronoi,regions,vertices,dict_lat_index_tot, dict_lat_index_tot_inv):
	...
	return M_adj_dist,aprox_angle(M_adj_angle)
...
def Preprocessing_PP(s,lower_M_p,lower_M,upper_M_p,upper_M):
	...
	N = len(lower_M)
	for i in range(0,12):
		cicly_consume_min[i] = nx.dijkstra_path_length(lower_M_p, source=SOURCE, target=i, weight='weight') + nx.dijkstra_path_length(lower_M, source=i, target=SOURCE, weight='weight')
		cicly_consume_max[i] = nx.dijkstra_path_length(upper_M_p, source=SOURCE, target=i, weight='weight') + nx.dijkstra_path_length(upper_M, source=i, target=SOURCE, weight='weight')
		path_l = nx.dijkstra_path(upper_M_p, source=SOURCE, target=i, weight='weight') + nx.dijkstra_path(upper_M, source=i, target=SOURCE, weight='weight')
		path_s =  nx.dijkstra_path(lower_M_p, source=SOURCE, target=i, weight='weight') + nx.dijkstra_path(lower_M, source=i, target=SOURCE, weight='weight')
...
def run_test_alg_off_sp(M_adj_dist,cells,point1,point2,wind_name,city,destination,prefixs,payload,status_v,indice_ora,M_index_to_name,M_name_to_coor):
	...
	M_adj_wind_gw = np.zeros((N,N))
	M_adj_energy = np.zeros((N,N))
	confronto_energetico = open("result/ggconfronto_energetico_off_sp"+str(indice_ora)+".txt","a+")
	consumption_go = 0
...
def run_test_alg_on_sp(M_adj_dist,cells,point1,point2,wind_name,city,destination,prefixs,payload,status_v,indice_ora,M_index_to_name,M_name_to_coor):
	...
	M_adj_wind_gw = np.zeros((N,N))
	M_adj_energy = np.zeros((N,N))
	confronto_energetico = open("result/ggconfronto_energetico_on_sp"+str(indice_ora)+".txt","a+")
	path_flag = True
	consumption_go = 0
	consumption_return= 0
	distance = 0
...
def run_test_alg_on_g(M_adj_dist,cells,point1,point2,wind_name,city,destination,prefixs,payload,status_v,indice_ora,M_index_to_name,M_name_to_coor):
	...
	M_adj_wind_gw = np.zeros((N,N))
	M_adj_energy = np.zeros((N,N))
	confronto_energetico = open("result/ggconfronto_energetico_on_g"+str(indice_ora)+".txt","a+")
	stop_flag = False
	consumption_go = 0
	consumption_return= 0
	distance = 0
...
def test_GDP(M_adj_dist,cells,point1,point2,wind_name,city,prefixs,indice_ora,PP_list,M_index_to_name,M_name_to_coor):
	...
	loc = "result/ggtest_alg_on_g"+str(indice_ora)+".txt"
	if(os.path.isfile(loc)):
		os.remove("result/ggtest_alg_on_g"+str(indice_ora)+".txt")
	loc = "result/ggconfronto_energetico_on_g"+str(indice_ora)+".txt"
	if(os.path.isfile(loc)):
		os.remove("result/ggconfronto_energetico_on_g"+str(indice_ora)+".txt")
	...
	file = open("result/ggtest_alg_on_g"+str(indice_ora)+".txt","a+")
	file.write("\n\n"+str(status))
	return final_status
...
def test_DSP(M_adj_dist,cells,point1,point2,wind_name,city,prefixs,indice_ora,PP_list,M_index_to_name,M_name_to_coor):
	...
	loc = "result/ggtest_alg_on_sp"+str(indice_ora)+".txt"
	if(os.path.isfile(loc)):
		os.remove("result/ggtest_alg_on_sp"+str(indice_ora)+".txt")
	loc = "result/ggconfronto_energetico_on_sp"+str(indice_ora)+".txt"
	if(os.path.isfile(loc)):
		os.remove("result/ggconfronto_energetico_on_sp"+str(indice_ora)+".txt")
	...
	file = open("result/ggtest_alg_on_sp"+str(indice_ora)+".txt","a+")
	file.write("\n\n"+str(status_o))
	return final_status
...
def test_OSP(M_adj_dist,cells,point1,point2,wind_name,city,prefixs,indice_ora,PP_list,M_index_to_name,M_name_to_coor):
	...
	final_status = {"CANCELED":0,"FAIL":0,"DELIVERED":0,"SUCCESS":0}
	status = {"CANCELED":0,"FAIL":0,"DELIVERED":0,"SUCCESS":0}
	loc = "result/ggtest_alg_off_sp"+str(indice_ora)+".txt"
	if(os.path.isfile(loc)):
		os.remove("result/ggtest_alg_off_sp"+str(indice_ora)+".txt")
	loc = "result/ggconfronto_energetico_off_sp"+str(indice_ora)+".txt"
	if(os.path.isfile(loc)):
		os.remove("result/ggconfronto_energetico_off_sp"+str(indice_ora)+".txt")
	...
	file = open("result/ggtest_alg_off_sp"+str(indice_ora)+".txt","a+")
	file.write("\n\n"+str(status))
	return final_status
