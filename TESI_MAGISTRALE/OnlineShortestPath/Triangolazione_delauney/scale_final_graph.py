...
mpmath.mp.dps = 5
BUDGET_ENERGY = 5000 #totale carica batteria Kj
...
def compute_prefixes():
	distance = 1 #un metro
	payload_weights = [0, 2, 7]
	drone_speeds = [10, 20]
	global_wind_speeds = [0, 5, 10, 15] #sto cambiando -- era [0, 5, 10, 15]
	relative_wind_directions = [0, 22.5, 45, 67.5, 90, 112.5, 135, 180]
	...
	for i in range(N):
		for j in range(N):
			...
			if M_adj_angle[i][j] <= 45 or M_adj_angle[i][j] >= 315:
				...
				M_aprox[i][j] = 180
			"""
			if M_adj_angle[i][j] <= 22.5 or M_adj_angle[i][j] >= 337.5:
				M_aprox[i][j] = 0
			elif M_adj_angle[i][j] <= 45 or M_adj_angle[i][j] >= 315:
				M_aprox[i][j] = 22.5
			elif M_adj_angle[i][j] <= 67.5 or M_adj_angle[i][j] >= 292.5:
				M_aprox[i][j] = 45
			elif M_adj_angle[i][j] <= 90 or M_adj_angle[i][j] >= 270:
				M_aprox[i][j] = 67.5
			elif M_adj_angle[i][j] <= 112.5 or M_adj_angle[i][j] >= 247.5:
				M_aprox[i][j] = 90
			elif M_adj_angle[i][j] <= 135 or M_adj_angle[i][j] >= 225:
				M_aprox[i][j] = 112.5
			elif M_adj_angle[i][j] <= 157.5 or M_adj_angle[i][j] >= 202.5:
				M_aprox[i][j] = 135
			else:
				M_aprox[i][j] = 180
		...
		for j in range(N):
			if M_adj_dist[i][j] != 0:
				if M_adj_wind[i][j] <= 4 and M_adj_wind[i][j] >= 0:
					M_aprox[i][j] = global_wind_speeds[0]
				elif M_adj_wind[i][j] <= 9 and M_adj_wind[i][j] > 4:
					M_aprox[i][j] = global_wind_speeds[1]
				elif M_adj_wind[i][j] <= 14 and M_adj_wind[i][j] > 9:
					M_aprox[i][j] = global_wind_speeds[2]
				else:
					M_aprox[i][j] = global_wind_speeds[3]
	return M_aprox
			...
			for i in range(N):
				for j in range(N):
					if(M_dist[i][j] != 0):
						...
						time = round((M_dist[i][j]*100/sp)/60,2)
						file1.write("EDGE LENGTH: "+str(round(M_dist[i][j],2))+"KM, TRAVERSING TIME: "+str(time)+"min, GLOBAL WIND DIRECTION: "+str(now_wind[i][j])+" RELATIVE WIND DIRECTION: "+str(M_angle[i][j])+", mu(e)*lunghezza: "+str(round(M_adj_dist[i][j]*100*get_energy(1,pl,sp,M_wind[i][j],M_angle[i][j]),2))+"KJ\n")
	return
def plot_table1(prefixes):
	distance = 1 #un metro
	payload_weights = [0, 2]
	drone_speeds = [0,5,10,15, 20]
	global_wind_speeds = [0, 5, 10, 15] #sto cambiando -- era [0, 5, 10, 15]
	relative_wind_directions = [0, 22.5, 45, 67.5, 90, 112.5, 135, 180]
	...
	for ds in range(0,21):
		print((ds,get_energy(distance,0,ds,0,0)))
		dataxs.append(ds)
		datays.append(round(get_energy(distance,2,ds,0,0),2))
	exit()
	ax1.plot(dataxs,datays, label="wind speed "+str(ds)+"m/s")
	plt.show()
	print(dataxs)
	print(datays)
	exit()
		...
		for j in range(N):
			if M_adj_dist[i][j] != 0:
				...
				if angleFromCoordinate(x0, y0, x1, y1)-winfo[2] < 0:
					M_adj_angle[i][j] = angleFromCoordinate(x0, y0, x1, y1)-windinfo[2]+360
				else:
					M_adj_angle[i][j] = angleFromCoordinate(x0, y0, x1, y1)-windinfo[2]
				if angleFromCoordinate(x1, y1, x0, y0)-winfo[2] <0:
					M_adj_angle[j][i] = angleFromCoordinate(x1, y1, x0, y0)-windinfo[2] +360
				else:
					M_adj_angle[j][i] = angleFromCoordinate(x1, y1, x0, y0)-windinfo[2]
...
def min_max_energy_matrix(M_dist,prefixs,speed,payload):
	...
	global_wind_speeds = [0, 5, 10, 15] #sto cambiando -- era [0, 5, 10, 15]
	relative_wind_directions = [0, 22.5, 45, 67.5, 90, 112.5, 135, 180]
	min_speed_direction_p = (100000,0,0)  
	for speed_i in global_wind_speeds:
		for dire in relative_wind_directions:
			if min_speed_direction_p[0] > round(get_energy(1,payload_weights[1],drone_speeds[0],speed_i,dire),2):
				min_speed_direction_p = (round(get_energy(1,payload_weights[1],drone_speeds[0],speed_i,dire),2),speed_i,dire)
	min_speed_direction = (100000,0,0)  
	for speed_i in global_wind_speeds:
		for dire in relative_wind_directions:
			if min_speed_direction[0] > round(get_energy(1,payload_weights[0],drone_speeds[0],speed_i,dire),2):
				min_speed_direction = (round(get_energy(1,payload_weights[0],drone_speeds[0],speed_i,dire),2),speed_i,dire)
	max_speed_direction_p = (0,0,0)  
	for speed_i in global_wind_speeds:
		for dire in relative_wind_directions:
			if max_speed_direction_p[0] < round(get_energy(1,payload_weights[1],drone_speeds[0],speed_i,dire),2):
				max_speed_direction_p = (round(get_energy(1,payload_weights[1],drone_speeds[0],speed_i,dire),2),speed_i,dire)
	max_speed_direction = (0,0,0)  
	for speed_i in global_wind_speeds:
		for dire in relative_wind_directions:
			if max_speed_direction[0] < round(get_energy(1,payload_weights[0],drone_speeds[0],speed_i,dire),2):
				max_speed_direction = (round(get_energy(1,payload_weights[0],drone_speeds[0],speed_i,dire),2),speed_i,dire)									
	...
	for i in range(12):
		for j in range(N):
			if M_dist[i][j] != 0:
				lower_bound_matrix[j][i] = (M_dist[j][i]*100) * prefixs[(0,speed,min_speed_direction[1],min_speed_direction[2])]
				lower_bound_matrix_p[j][i] = (M_dist[j][i]*100) * prefixs[(payload,speed,min_speed_direction_p[1],min_speed_direction_p[2])]
				...
				upper_bound_matrix[j][i] = (M_dist[j][i]*100) * prefixs[(0,speed,max_speed_direction[1],max_speed_direction[2])]
				upper_bound_matrix_p[j][i] = (M_dist[j][i]*100) * prefixs[(payload,speed,max_speed_direction_p[1],max_speed_direction_p[2])]
...
def Preprocessing_PP(s,lower_M_p,lower_M,upper_M_p,upper_M):
	...
	cicly_consume_max = {}
	colors = []
	loc = "result/sPreprocessing_PP.txt"
	if(os.path.isfile(loc)):
		os.remove("result/sPreprocessing_PP.txt")
	file = open("result/sPreprocessing_PP.txt","a+")
	N = len(lower_M)
	for i in range(0,12):
...
def run_test_alg_off_sp(M_adj_dist,M_index_to_name,M_name_to_coor,destination,prefixs,payload,status_v,indice_ora):
	...
	M_adj_wind_gw = np.zeros((N,N))
	M_adj_energy = np.zeros((N,N))
	file = open("result/stest_alg_off_sp"+str(indice_ora)+".txt","a+")
	confronto_energetico = open("result/sconfronto_energetico_off_sp"+str(indice_ora)+".txt","a+")
	consumption_go = 0
	...
	if cycle_cost <= BUDGET_ENERGY:
		while node != destination:
			...
			if control_time == 0:
				...
				file.write("->(" + M_index_to_name[node] + "," + M_index_to_name[node_next] + "," + str(round(M_adj_dist[node][node_next],2)) + "," + str(round((M_adj_dist[node][node_next]*100/DRONE_SPEED)/60,2)) + "," + str(round(G[node][node_next]['weight'],2)) + "," + str(M_adj_angle[node][node_next]) + "," + str(M_adj_wind[node][node_next])+")")
				node = node_next
			else:
		...
		if consumption_go <= BUDGET_ENERGY:
			while node_r != SOURCE:
				...
				if control_time == 0:
					...
					file.write("->("+M_index_to_name[node_r]+","+M_index_to_name[node_r_next]+","+str(round(M_adj_dist[node_r][node_r_next],2))+","+str(round((M_adj_dist[node_r][node_r_next]*100/DRONE_SPEED)/60,2))+","+str(round(G1[node_r][node_r_next]['weight'],2))+","+str(M_adj_angle[node_r][node_r_next])+","+str(M_adj_wind[node_r][node_r_next])+")")
					node_r = node_r_next
				else:
					indice_orario = (indice_orario + int(control_time)) % 240
					if indice_orario <= 1:
		...
		status_v[STATUS] = status_v[STATUS] +1
		confronto_energetico.write("\nABORT")
	file.write("\nMISSION STATUS: "+STATUS+" WIND CHANGES:"+str(time // 15)+"\n\n")
	#print(STATUS)
...
def run_test_alg_on_sp(M_adj_dist,M_index_to_name,M_name_to_coor,destination,prefixs,payload,status_v,indice_ora):
	...
	M_adj_wind_gw = np.zeros((N,N))
	M_adj_energy = np.zeros((N,N))
	file = open("result/stest_alg_on_sp"+str(indice_ora)+".txt","a+")
	confronto_energetico = open("result/sconfronto_energetico_on_sp"+str(indice_ora)+".txt","a+")
	path_flag = True
	consumption_go = 0
	consumption_return= 0
	distance = 0
...
def run_test_alg_on_g(M_adj_dist,M_index_to_name,M_name_to_coor,destination,prefixs,payload,status_v,indice_ora):
	...
	M_adj_wind_gw = np.zeros((N,N))
	M_adj_energy = np.zeros((N,N))
	file = open("result/stest_alg_on_g"+str(indice_ora)+".txt","a+")
	confronto_energetico = open("result/sconfronto_energetico_on_g"+str(indice_ora)+".txt","a+")
	stop_flag = False
	consumption_go = 0
	consumption_return= 0
	distance = 0
	...
	file.close()
	return status_v
def test_GDP(M_adj_dist,M_index_to_name,M_name_to_coor,prefixs,indice_ora,PP_list):
	fail = 0
	succes = 0
	delivered = 0
	...
	loc = "result/stest_alg_on_g"+str(indice_ora)+".txt"
	if(os.path.isfile(loc)):
		os.remove("result/stest_alg_on_g"+str(indice_ora)+".txt")
	loc = "result/sconfronto_energetico_on_g"+str(indice_ora)+".txt"
	if(os.path.isfile(loc)):
		os.remove("result/sconfronto_energetico_on_g"+str(indice_ora)+".txt")
	...
	for destination in PP_list:
		...
		fail = fail + status_g["FAIL"]
		delivered = delivered + status_g["DELIVERED"]
		succes = succes + status_g["SUCCESS"]
...
def test_DSP(M_adj_dist,M_index_to_name,M_name_to_coor,prefixs,indice_ora,PP_list):
	fail = 0
	succes = 0
	delivered = 0
	tic = time.perf_counter()
	...
	final_status = {"FAIL":0,"DELIVERED":0,"SUCCESS":0}
	loc = "result/stest_alg_on_sp"+str(indice_ora)+".txt"
	if(os.path.isfile(loc)):
		os.remove("result/stest_alg_on_sp"+str(indice_ora)+".txt")
	loc = "result/sconfronto_energetico_on_sp"+str(indice_ora)+".txt"
	if(os.path.isfile(loc)):
		os.remove("result/sconfronto_energetico_on_sp"+str(indice_ora)+".txt")
...
def test_OSP(M_adj_dist,M_index_to_name,M_name_to_coor,prefixs,indice_ora,PP_list):
	tic = time.perf_counter()
	fail = 0
	succes = 0
	delivered = 0
	...
	loc = "result/stest_alg_off_sp"+str(indice_ora)+".txt"
	if(os.path.isfile(loc)):
		os.remove("result/stest_alg_off_sp"+str(indice_ora)+".txt")
	loc = "result/sconfronto_energetico_off_sp"+str(indice_ora)+".txt"
	if(os.path.isfile(loc)):
		os.remove("result/sconfronto_energetico_off_sp"+str(indice_ora)+".txt")
...
def n_random_test(n,M_adj_dist,M_adj_angle,M_adj_wind,prefixs):
	...
	for i in indici_orari_test:
		final_g = Counter(final_g)+ Counter(test_GDP(M_adj_dist,M_index_to_name,M_name_to_coor,prefixs,i,gray_list))
		#final_d = Counter(final_d)+Counter(test_DSP(M_adj_dist,M_index_to_name,M_name_to_coor,prefixs,i,gray_list))
		#final_o = Counter(final_o)+Counter(test_OSP(M_adj_dist,M_index_to_name,M_name_to_coor,prefixs,i,gray_list))
...
if __name__ == "__main__":
	...
	#print("Diametro del grafo: "+str(maxD))
